...
class ADNI_3D(data.Dataset):
    ...
    def __init__(self, dir_to_scans, dir_to_tsv, mode = 'Train', n_label = 3, percentage_usage = 1.0):
        ...
        for i in range(len(subject_tsv)):
            path_check = os.path.join(dir_to_scans, subject_tsv.iloc[i].participant_id,
                                      subject_tsv.iloc[i].session_id, 't1/spm/segmentation/normalized_space')
            if os.path.exists(path_check) and (subject_tsv.iloc[i].diagnosis in LABEL_MAPPING):
